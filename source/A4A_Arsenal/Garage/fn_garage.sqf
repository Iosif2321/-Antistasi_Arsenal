/*
    ...
    All-in-one function: called with ["mode", params] call A4A_fnc_garage

    Data model per garage (stored on `server` object as A4A_GRG_<garageID>):
    ...
    Persistence: profileNamespace key "A4A_GarageData_<garageID>"
*/
...
case "initserver": {
    ...
    private _profileKey = format ["A4A_GarageData_%1", _garageID];
    private _saved = profileNamespace getVariable [_profileKey, []];

    private _data = [];
    for "_i" from 0 to (A4A_GRG_NUM_CATS - 1) do { _data pushBack createHashMap };

    // Load saved data
    if (count _saved == A4A_GRG_NUM_CATS) then {
        {
    ...
    // UID counter
    if (isNil {server getVariable format ["A4A_GRG_UID_%1", _garageID]}) then {
        server setVariable [format ["A4A_GRG_UID_%1", _garageID], 0, true];
    };
    ...
    diag_log format ["A4A_Garage: Initialized garage '%1' with %2 vehicles", _garageID, _totalVeh];
...
case "save": {
    ...
    if (count _data != A4A_GRG_NUM_CATS) exitWith {};
    ...
    profileNamespace setVariable [format ["A4A_GarageData_%1", _garageID], _toSave];
    saveProfileNamespace;
    diag_log format ["A4A_Garage: Saved garage '%1'", _garageID];
};
...
case "nextuid": {
    ...
    _params params ["_garageID"];
    private _key = format ["A4A_GRG_UID_%1", _garageID];
...
case "open": {
    private _garageObj = missionNamespace getVariable ["A4A_GRG_object", objNull];
    private _garageID = _garageObj getVariable ["A4A_Garage_ID", "Default"];

    // Ensure server has initialized this garage
    ["initServer", [_garageID]] remoteExecCall ["A4A_fnc_garage", 2];

    // Store current garage context
    uiNamespace setVariable ["A4A_GRG_currentID", _garageID];
    uiNamespace setVariable ["A4A_GRG_currentObj", _garageObj];
    uiNamespace setVariable ["A4A_GRG_currentCat", 0];

    // Open dialog
    createDialog "A4A_GRG_Dialog";
};
...
case "onload": {
    ...
    uiNamespace setVariable ["A4A_GRG_display", _display];

    private _garageID = uiNamespace getVariable ["A4A_GRG_currentID", "Default"];
    (_display displayCtrl A4A_GRG_IDC_TITLE) ctrlSetText format ["Garage: %1", _garageID];

    // Small delay to let server init complete, then load first category
    [] spawn {
        uiSleep 0.5;
        ["switchCat", [0]] call A4A_fnc_garage;
        ["updateCount"] call A4A_fnc_garage;
    };
};
...
case "onunload": {
    uiNamespace setVariable ["A4A_GRG_display", nil];
    uiNamespace setVariable ["A4A_GRG_selectedUID", nil];
    uiNamespace setVariable ["A4A_GRG_selectedCat", nil];
};

///////////////////////////////////////////////////////////////////////////////////////////
// CLIENT: Close dialog
case "close": {
    private _display = uiNamespace getVariable ["A4A_GRG_display", displayNull];
...
case "switchcat": {
    ...
    uiNamespace setVariable ["A4A_GRG_currentCat", _catIdx];

    private _display = uiNamespace getVariable ["A4A_GRG_display", displayNull];
    if (isNull _display) exitWith {};

    // Hide all listboxes, show selected
    private _catIDCs = [A4A_GRG_IDC_CAT_CARS, A4A_GRG_IDC_CAT_ARMOR, A4A_GRG_IDC_CAT_HELI, A4A_GRG_IDC_CAT_PLANE, A4A_GRG_IDC_CAT_BOAT, A4A_GRG_IDC_CAT_STATIC];
    private _btnIDCs = [A4A_GRG_IDC_BTN_CARS, A4A_GRG_IDC_BTN_ARMOR, A4A_GRG_IDC_BTN_HELI, A4A_GRG_IDC_BTN_PLANE, A4A_GRG_IDC_BTN_BOAT, A4A_GRG_IDC_BTN_STATIC];
    {
    ...
    // Populate list
    ["reloadList", [_catIdx]] call A4A_fnc_garage;

    // Clear selection
    uiNamespace setVariable ["A4A_GRG_selectedUID", nil];
    (_display displayCtrl A4A_GRG_IDC_INFO) ctrlSetStructuredText parseText "";
    (_display displayCtrl A4A_GRG_IDC_BTN_RETRIEVE) ctrlEnable false;
};
...
case "reloadlist": {
    ...
    private _garageID = uiNamespace getVariable ["A4A_GRG_currentID", "Default"];
    private _data = ["getData", [_garageID]] call A4A_fnc_garage;
    if (count _data != A4A_GRG_NUM_CATS) exitWith {};
...
case "updatecount": {
    ...
    (_display displayCtrl A4A_GRG_IDC_COUNT) ctrlSetText format ["%1 vehicles", _total];
};
...
case "selchanged": {
    ...
    private _catIdx = uiNamespace getVariable ["A4A_GRG_currentCat", 0];

    uiNamespace setVariable ["A4A_GRG_selectedUID", _uid];
    uiNamespace setVariable ["A4A_GRG_selectedCat", _catIdx];
    ...
    (_display displayCtrl A4A_GRG_IDC_INFO) ctrlSetStructuredText parseText _info;

    // Enable retrieve button
    (_display displayCtrl A4A_GRG_IDC_BTN_RETRIEVE) ctrlEnable true;
};

///////////////////////////////////////////////////////////////////////////////////////////
// CLIENT: Store nearby vehicle into garage
case "storevehicle": {
    private _garageObj = uiNamespace getVariable ["A4A_GRG_currentObj", objNull];
    private _garageID = uiNamespace getVariable ["A4A_GRG_currentID", "Default"];
    ...
    // Send to server
    ["addVehicle", [_garageID, _vehicle, clientOwner]] remoteExecCall ["A4A_fnc_garage", 2];

    // Refresh after short delay
    [] spawn {
        uiSleep 1;
        private _catIdx = uiNamespace getVariable ["A4A_GRG_currentCat", 0];
        ["switchCat", [_catIdx]] call A4A_fnc_garage;
    };
};
...
case "retrievevehicle": {
    private _uid = uiNamespace getVariable ["A4A_GRG_selectedUID", nil];
    private _catIdx = uiNamespace getVariable ["A4A_GRG_selectedCat", nil];
    ...
    [_garageID, _catIdx, _uid, _garageObj] spawn {
        ...
        private _vehData = ["removeVehicle", [_garageID, _catIdx, _uid]] call A4A_fnc_garage;
...
///////////////////////////////////////////////////////////////////////////////////////////
default {
    diag_log format ["A4A_Garage: Unknown mode '%1'", _mode];
};
