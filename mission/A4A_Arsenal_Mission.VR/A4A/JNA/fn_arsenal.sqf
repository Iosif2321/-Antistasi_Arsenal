...
// The dispatcher is a local UI helper. Every network operation uses a narrow
// mission-native A4A endpoint and never enters this switch remotely.
private _a4aIsRemote = isRemoteExecuted;
private _a4aRemoteOwner = if (_a4aIsRemote) then {remoteExecutedOwner} else {-1};
if (_a4aIsRemote) exitWith {
	diag_log format ["A4A Mission: rejected remote UI dispatcher call from owner %1", _a4aRemoteOwner];
};
...
switch _mode do {
	...
	case "CustomInit":{
		...
		// Guard: only end loading screen if it was actually started
		private _lsIds = missionNamespace getVariable ["BIS_fnc_startLoadingScreen_ids", []];
		if ("jn_fnc_arsenal" in _lsIds) then {
	...
	case "customEvents":{
		...
		if (!isNil "A4A_fnc_canEdit" && {[player] call A4A_fnc_canEdit}) then {
			...
			_ctrlButtonExport ctrlSetFade 0;
			...
			_ctrlButtonExport ctrlAddEventHandler ["buttonclick", {
				...
				if (!isNil {uiNamespace getVariable "jna_editor_open"}) then {
					["EditorClose", [_display]] call jn_fnc_arsenal;
				} else {
					["EditorOpen", [_display]] call jn_fnc_arsenal;
				};
			}];
		} else {
			_ctrlButtonExport ctrlSetText "";
			_ctrlButtonExport ctrlEnable false;
			_ctrlButtonExport ctrlSetFade 1;
			_ctrlButtonExport ctrlCommit 0;
		};
		...
		_ctrlButtonRandom ctrlShow false;
		...
		switch (_sortType) do {
			case SORT_ALPHABETICAL: {
				...
				for "_i" from (_itemCount - 1) to 0 step -1 do {
					...
					if (_dataStr != "") then {
						private _data = if (_dataStr select [0,1] == "[") then {call compile _dataStr} else {[_dataStr,0,"",""]};
	...
	case "TabSelectLeft": {
		...
		["LocalItemAdd",[_index,_inventory_player,0]] call jn_fnc_arsenal;
	...
	case "TabSelectRight": {
		...
		switch _index do {
			...
			case IDC_RSCDISPLAYARSENAL_TAB_ITEMBIPOD: {
				...
				["LocalItemAdd",[_index,_item,0]] call jn_fnc_arsenal;
					...
					{
						{
							if([_itemsUnique, _x] call _arrayContains)then{
								["LocalItemAdd",[_index,_x,0]] call jn_fnc_arsenal;
							}
	...
	/////////////////////////////////////////////////////////////////////////////////////////// Local list helper
	case "LocalItemAdd":{
		params ["_index", "_item", ["_amount", 0]];
		if (_a4aIsRemote || {!hasInterface}) exitWith {};
		if !(
		...
		private _isListNBox = ctrlType _ctrlList == 102;
		if ((_index in [IDCS_RIGHT]) && {!(ctrlEnabled _ctrlList)}) exitWith {};

		private _foundRow = -1;
		private _rowSize = if (_isListNBox) then {(lnbSize _ctrlList select 0) - 1} else {lbSize _ctrlList - 1};
		for "_row" from 0 to _rowSize do {
			private _dataString = if (_isListNBox) then {_ctrlList lnbData [_row, 0]} else {_ctrlList lbData _row};
			private _currentData = if (_dataString select [0, 1] == "[") then {call compile _dataString} else {[_dataString, 0, "", ""]};
			private _currentItem = _currentData param [0, ""];
			if (_item isEqualTo _currentItem) exitWith {
				_foundRow = _row;
				private _currentAmount = _currentData param [1, 0];
				private _displayName = _currentData param [2, ""];
				private _newAmount = if (_amount == -1 || {_currentAmount == -1}) then {-1} else {_currentAmount + _amount};
				private _updatedData = str [_item, _newAmount, _displayName];
				if (_isListNBox) then {
					_ctrlList lnbSetData [[_row, 0], _updatedData];
				} else {
					_ctrlList lbSetData [_row, _updatedData];
				};
			};
		};
		...
		if (_foundRow == -1) then {
			["CreateItem", [_display, _ctrlList, _index, _item, _amount]] call jn_fnc_arsenal;
			_foundRow = _rowSize + 1;
		};
		["UpdateItemGui", [_display, _ctrlList, _index, _foundRow]] call jn_fnc_arsenal;
	};
	...
	case "ExportData": {
		...
		private _uniqueItems = 0;
		{ { _totalItems = _totalItems + (_x select 1); _uniqueItems = _uniqueItems + 1 } forEach _x } forEach jna_dataList;

		// Helper: get displayName from config (may contain Unicode/Cyrillic)
		private _getDisplayName = {
			...
			if (isClass (configFile >> "CfgWeapons" >> _cls)) then {
				...
				if (isClass (configFile >> "CfgMagazines" >> _cls)) then {
					_dn = getText (configFile >> "CfgMagazines" >> _cls >> "displayName");
				} else {
					if (isClass (configFile >> "CfgVehicles" >> _cls)) then {
						_dn = getText (configFile >> "CfgVehicles" >> _cls >> "displayName");
					} else {
						if (isClass (configFile >> "CfgGlasses" >> _cls)) then {
							_dn = getText (configFile >> "CfgGlasses" >> _cls >> "displayName");
						};
					};
				};
			};
			if (_dn isEqualTo "") then { _dn = "?" };
			_dn
		};

		// Build formatted output for clipboard (Unicode-safe, displayNames included)
		private _lines = [];
		_lines pushBack "// ============================================================";
		_lines pushBack format ["// Antistasi Arsenal Export - ID: '%1'", _arsenalID];
		_lines pushBack format ["// %1 unique items, %2 total count", _uniqueItems, _totalItems];
		_lines pushBack format ["// Profile: %1", profileName];
		...
		// Per-category listing (clipboard  with displayName)
		{
			...
			if (count _x > 0) then {
				_lines pushBack format ["// --- [%1] %2 (%3 items) ---", _catIndex, _catName, count _x];
				{
					_x params ["_cls", "_cnt"];
		...
		// Raw SQF data block (importable)
		_lines pushBack "// === BEGIN DATA ===";
		_lines pushBack str jna_dataList;
		_lines pushBack "// === END DATA ===";
		...
		diag_log format ["// Antistasi Arsenal Export - '%1' - %2 unique items", _arsenalID, _uniqueItems];
		{
			private _catIndex = _forEachIndex;
			if (count _x > 0) then {
				private _catName = if (_catIndex < count _catNames) then { _catNames select _catIndex } else { str _catIndex };
				diag_log format ["// [%1] %2:", _catIndex, _catName];
				{ diag_log format ["    %1  x%2", _x select 0, _x select 1] } forEach _x;
			};
		} forEach jna_dataList;
		diag_log "// ============================================================";
		...
		if (!_rptOnly) then {
			...
			systemChat (localize "STR_JNA_ACT_EXPORT_OK");
			...
			diag_log format ["// Arsenal '%1': %2 items auto-logged on close.", _arsenalID, _uniqueItems];
		};
	};
	...
	case "ImportData": {
		...
		if (_clipText isEqualTo "") exitWith {
			...
			hint "Clipboard is empty. Copy arsenal data first.";
		};

		// Try to extract data between markers if present
		private _dataText = _clipText;
		private _beginIdx = _clipText find "// === BEGIN DATA ===";
		if (_beginIdx >= 0) then {
			_dataText = _clipText select [_beginIdx + 21];
			private _endIdx = _dataText find "// === END DATA ===";
			if (_endIdx >= 0) then {
				_dataText = _dataText select [0, _endIdx];
			};
		};
		...
		if !(_parsed isEqualType []) exitWith {
			systemChat "Import failed: parsed data is not an array.";
			hint (localize "STR_JNA_ACT_IMPORT_FORMAT");
		};
		if (count _parsed != 27) exitWith {
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	// ==================== ARSENAL EDITOR (Zeus-only) ====================
	// Full in-arsenal content editor with category tabs, item list, +/-/INF buttons
	///////////////////////////////////////////////////////////////////////////////////////////
	...
	case "EditorOpen": {
		...
		uiNamespace setVariable ["jna_editor_open", true];

		// Initialize saved counts hashmap (for INF toggle memory)
		if (isNil {uiNamespace getVariable "jna_editor_savedCounts"}) then {
		...
		private _panelY = safeZoneY + safeZoneH * 0.05;
		private _panelW = safeZoneW * 0.70;
		private _panelH = safeZoneH * 0.90;
		private _titleH = safeZoneH * 0.04;
		private _tabH   = safeZoneH * 0.045;
		private _btnW   = safeZoneW * 0.06;
		private _btnH   = safeZoneH * 0.04;
		private _pad    = safeZoneW * 0.005;

		private _createdCtrls = [];

		// --- Background ---
		private _bg = _display ctrlCreate ["RscText", 90000];
		_bg ctrlSetPosition [_panelX, _panelY, _panelW, _panelH];
		_bg ctrlSetBackgroundColor [0, 0, 0, 0.85];
		_bg ctrlCommit 0;
		_createdCtrls pushBack 90000;

		// --- Title ---
		private _title = _display ctrlCreate ["RscText", 90001];
		_title ctrlSetPosition [_panelX + _pad, _panelY + _pad, _panelW - _pad * 2 - _btnW, _titleH];
		_title ctrlSetText "Arsenal Editor";
		_title ctrlSetBackgroundColor [0.2, 0.2, 0.2, 1];
		_title ctrlCommit 0;
		_createdCtrls pushBack 90001;

		// --- Close button ---
		private _btnClose = _display ctrlCreate ["RscButton", 90006];
		_btnClose ctrlSetPosition [_panelX + _panelW - _pad - _btnW, _panelY + _pad, _btnW, _titleH];
		_btnClose ctrlSetText "X";
		_btnClose ctrlSetBackgroundColor [0.5, 0.1, 0.1, 1];
		_btnClose ctrlAddEventHandler ["ButtonClick", {
			["EditorClose", [ctrlParent (_this select 0)]] call jn_fnc_arsenal;
		}];
		_btnClose ctrlCommit 0;
		_createdCtrls pushBack 90006;

		// --- Category tabs ---
		private _tabY = _panelY + _pad + _titleH + _pad;
		private _tabW = ((_panelW - _pad * 2) / (count _categories)) min (safeZoneW * 0.038);
		{
			...
			_createdCtrls pushBack _idc;
		} forEach _categories;

		// --- Item list (ListBox) ---
		private _listY = _tabY + _tabH + _pad;
		private _listW = _panelW - _pad * 3 - _btnW;
		private _listH = _panelY + _panelH - _listY - _pad;
		private _list = _display ctrlCreate ["RscListBox", 90002];
		_list ctrlSetPosition [_panelX + _pad, _listY, _listW, _listH];
		_list ctrlSetBackgroundColor [0.1, 0.1, 0.1, 0.9];
		_list ctrlCommit 0;
		_createdCtrls pushBack 90002;

		// --- Action buttons column (right side) ---
		private _abX = _panelX + _panelW - _pad - _btnW;
		private _abY = _listY;

		// (+) button
		private _btnPlus = _display ctrlCreate ["RscButton", 90004];
		_btnPlus ctrlSetPosition [_abX, _abY, _btnW, _btnH];
		_btnPlus ctrlSetText "+";
		_btnPlus ctrlSetBackgroundColor [0.1, 0.4, 0.1, 1];
		_btnPlus ctrlAddEventHandler ["ButtonClick", {
			["EditorModify", [ctrlParent (_this select 0), 1]] call jn_fnc_arsenal;
		}];
		_btnPlus ctrlCommit 0;
		_createdCtrls pushBack 90004;

		// (-) button
		private _btnMinus = _display ctrlCreate ["RscButton", 90003];
		_btnMinus ctrlSetPosition [_abX, _abY + _btnH + _pad, _btnW, _btnH];
		_btnMinus ctrlSetText "-";
		_btnMinus ctrlSetBackgroundColor [0.4, 0.1, 0.1, 1];
		_btnMinus ctrlAddEventHandler ["ButtonClick", {
			["EditorModify", [ctrlParent (_this select 0), -1]] call jn_fnc_arsenal;
		}];
		_btnMinus ctrlCommit 0;
		_createdCtrls pushBack 90003;

		// (INF) button
		private _btnInf = _display ctrlCreate ["RscButton", 90005];
		_btnInf ctrlSetPosition [_abX, _abY + (_btnH + _pad) * 2, _btnW, _btnH];
		_btnInf ctrlSetText "INF";
		_btnInf ctrlSetBackgroundColor [0.3, 0.3, 0.1, 1];
		_btnInf ctrlAddEventHandler ["ButtonClick", {
			["EditorModify", [ctrlParent (_this select 0), 0]] call jn_fnc_arsenal;
		}];
		_btnInf ctrlCommit 0;
		_createdCtrls pushBack 90005;

		// --- Count display ---
		private _countText = _display ctrlCreate ["RscText", 90200];
		_countText ctrlSetPosition [_abX, _abY + (_btnH + _pad) * 3 + _pad, _btnW, _btnH];
		_countText ctrlSetText "---";
		_countText ctrlSetBackgroundColor [0.15, 0.15, 0.15, 1];
		_countText ctrlCommit 0;
		_createdCtrls pushBack 90200;

		// --- Save button ---
		private _btnSave = _display ctrlCreate ["RscButton", 90007];
		_btnSave ctrlSetPosition [_abX, _abY + (_btnH + _pad) * 4 + _pad, _btnW, _btnH];
		_btnSave ctrlSetText "SAVE";
		_btnSave ctrlSetBackgroundColor [0.1, 0.3, 0.5, 1];
		_btnSave ctrlAddEventHandler ["ButtonClick", {
			["EditorSave", [ctrlParent (_this select 0)]] call jn_fnc_arsenal;
		}];
		_btnSave ctrlCommit 0;
		_createdCtrls pushBack 90007;

		// Store created controls for cleanup and select first category
		uiNamespace setVariable ["jna_editor_ctrls", _createdCtrls];
		uiNamespace setVariable ["jna_editor_currentCat", (_categories select 0) select 0];

		// List selection handler
		(_display displayCtrl 90002) ctrlAddEventHandler ["LBSelChanged", {
			["EditorUpdateCount", [ctrlParent (_this select 0)]] call jn_fnc_arsenal;
		}];
	...
	case "EditorClose": {
		...
		private _ctrls = uiNamespace getVariable ["jna_editor_ctrls", []];
		{ ctrlDelete (_display displayCtrl _x) } forEach _ctrls;

		uiNamespace setVariable ["jna_editor_open", nil];
		uiNamespace setVariable ["jna_editor_ctrls", nil];
		uiNamespace setVariable ["jna_editor_currentCat", nil];
	};

	///////////////////////////////////////////////////////////////////////////////////////////
	case "EditorSelectCat": {
		_this params ["_display", "_catIdx"];
		disableSerialization;

		uiNamespace setVariable ["jna_editor_currentCat", _catIdx];
		...
		{
			_x params ["_cls", "_cnt"];
			private _cfg = [_cls, _catIdx] call _getItemConfig;
			private _displayName = getText (_cfg >> "displayName");
			if (_displayName isEqualTo "") then { _displayName = _cls };
			private _cntStr = if (_cnt == -1) then { "INF" } else { str _cnt };
			private _lbIdx = _list lbAdd format ["%1  [%2]", _displayName, _cntStr];
			_list lbSetData [_lbIdx, _cls];
			_list lbSetPicture [_lbIdx, getText (_cfg >> "picture")];
			if (_cnt == -1) then {
				_list lbSetColor [_lbIdx, [0.2, 0.9, 0.2, 1]];
			} else {
				if (_cnt > 0) then {
					_list lbSetColor [_lbIdx, [1, 1, 1, 1]];
				} else {
					_list lbSetColor [_lbIdx, [0.5, 0.5, 0.5, 1]];
				};
			};
			_addedClasses set [_cls, true];
		} forEach _arsenalItems;

		// Then: all config items not yet in arsenal (count = 0)
		{
			if !(_x in _addedClasses) then {
				private _cfg = [_x, _catIdx] call _getItemConfig;
				private _displayName = getText (_cfg >> "displayName");
				if (_displayName isEqualTo "") then { _displayName = _x };
				private _lbIdx = _list lbAdd format ["%1  [0]", _displayName];
				_list lbSetData [_lbIdx, _x];
				_list lbSetPicture [_lbIdx, getText (_cfg >> "picture")];
		...
		(_display displayCtrl 90200) ctrlSetText "---";
	};

	///////////////////////////////////////////////////////////////////////////////////////////
	case "EditorUpdateCount": {
		private _display = _this select 0;
		disableSerialization;

		private _list = _display displayCtrl 90002;
		private _sel = lbCurSel _list;
		if (_sel < 0) exitWith { (_display displayCtrl 90200) ctrlSetText "---" };

		private _cls = _list lbData _sel;
		private _catIdx = uiNamespace getVariable ["jna_editor_currentCat", 0];
		private _arsenalItems = jna_dataList param [_catIdx, []];
		private _cnt = 0;
		{ if ((_x select 0) isEqualTo _cls) exitWith { _cnt = _x select 1 } } forEach _arsenalItems;

		private _cntStr = if (_cnt == -1) then { "INF" } else { str _cnt };
		(_display displayCtrl 90200) ctrlSetText _cntStr;
	};

	///////////////////////////////////////////////////////////////////////////////////////////
	// _action: 1 = plus, -1 = minus, 0 = toggle INF
	case "EditorModify": {
		...
		private _catIdx = uiNamespace getVariable ["jna_editor_currentCat", 0];
		private _arsenalItems = +(jna_dataList param [_catIdx, []]);
		private _savedCounts = uiNamespace getVariable ["jna_editor_savedCounts", createHashMap];

		// Find item in arsenal list
		private _found = false;
		private _itemIdx = -1;
		private _currentCnt = 0;
		{
			if ((_x select 0) isEqualTo _cls) exitWith {
				_found = true;
				_itemIdx = _forEachIndex;
				_currentCnt = _x select 1;
			};
		} forEach _arsenalItems;

		private _newCnt = _currentCnt;

		switch true do {
			// Toggle INF
			case (_action == 0): {
				if (_currentCnt == -1) then {
					// Disable INF: restore saved count (or 1 if none saved)
					_newCnt = _savedCounts getOrDefault [_cls, 1];
					_savedCounts deleteAt _cls;
				} else {
					// Enable INF: save current count, set to -1
					if (_currentCnt > 0) then {
						_savedCounts set [_cls, _currentCnt];
					} else {
						_savedCounts set [_cls, 1];
					};
					_newCnt = -1;
				};
			};
			...
			case (_action > 0): {
				if (_currentCnt == -1) then {
					_newCnt = -1; // already INF, stay INF
				} else {
					_newCnt = _currentCnt + 1;
				};
			};
			// Minus
			case (_action < 0): {
				if (_currentCnt == -1) then {
					// From INF, go to saved count
					_newCnt = _savedCounts getOrDefault [_cls, 1];
					_savedCounts deleteAt _cls;
				} else {
					_newCnt = (_currentCnt - 1) max 0;
				};
			};
		};

		uiNamespace setVariable ["jna_editor_savedCounts", _savedCounts];

		// Update jna_dataList
		if (_newCnt > 0 || _newCnt == -1) then {
			if (_found) then {
				_arsenalItems set [_itemIdx, [_cls, _newCnt]];
			} else {
				_arsenalItems pushBack [_cls, _newCnt];
			};
		} else {
			// Remove item (count = 0)
			if (_found) then {
				_arsenalItems deleteAt _itemIdx;
			};
		};

		jna_dataList set [_catIdx, _arsenalItems];

		// Refresh list
		["EditorSelectCat", [_display, _catIdx]] call jn_fnc_arsenal;

		// Re-select the same item
		private _newList = _display displayCtrl 90002;
		for "_i" from 0 to (lbSize _newList - 1) do {
			if ((_newList lbData _i) isEqualTo _cls) exitWith {
				_newList lbSetCurSel _i;
			};
		};
	};
