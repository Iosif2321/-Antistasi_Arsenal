...
if !(
    ...
    {(_transaction getOrDefault ["kind", ""]) isEqualTo "return"} &&
    {(_transaction getOrDefault ["state", ""]) isEqualTo "pending"} &&
...
private _payload = [_transactionId, "return", _success, _generation, _revision, _snapshot, _rollback, _message];
