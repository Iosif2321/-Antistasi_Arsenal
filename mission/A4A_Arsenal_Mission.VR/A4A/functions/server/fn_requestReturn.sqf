...
private _sendResult = {
    ...
    private _payload = [_transactionId, "return", false, _generation, _revision, _snapshot, true, _message];
...
if (!isNil {_transactions get _transactionId}) exitWith {
    ...
    if (
        ...
        {(_existing getOrDefault ["kind", ""]) isEqualTo "return"}
    ) then {
        private _payload = [_transactionId, "return", _generation, _revision, _index, _item, _amount];
