...
- [ ] Add a failing compatibility test proving there is no hard CBA dependency, `CBA_fnc_addSetting` is optional, no nonexistent `CBA_fnc_getSetting` call remains, and settings are read from the variable generated by CBA Settings with mission defaults as fallback.
- [ ] Run the test and observe failure.
- [ ] Implement idempotent optional setting registration and private server snapshots for authority-sensitive values.
- [ ] Compare used symbols with the locally installed CBA_A3 3.19.0 source and rerun the test.
- [ ] Commit exact paths with message `feat: add optional cba settings adapter`.
